...
bool ivf_kmeans(
    ...
    const int* h_indices_in,       // [n] CPU nullptr
    int* h_indices_out             // [n] CPU nullptr
) {
    ...
    if (use_minibatch) {
        ...
        if (gpu_count > 1 && n >= 1000000) {
            gpu_kmeans_lloyd_multigpu(cfg, h_data_in, d_assign, d_centroids, &obj);
        } else {
    ...
    // Step 3: Build permutation on GPU ( H2D )
