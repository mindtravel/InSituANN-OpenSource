...
static inline float l2_sq_v1(const float* __restrict__ q,
...
static inline float cos_dist_v1(const float* __restrict__ q,
                                const float* __restrict__ v, int dim) {
    ...
    for (int qi = 0; qi < n_query; ++qi) {
        ...
        for (int pi = 0; pi < n_probes; ++pi) {
            ...
            for (int vi = 0; vi < count; ++vi) {
                ...
                float d = (distance_mode == 0) ? l2_sq_v1(q, v, dim)
                                               : cos_dist_v1(q, v, dim);
...
const char* variant_name_v1() { return "V1_scalar_O3_march_native"; }
