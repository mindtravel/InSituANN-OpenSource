...
)
{
    ...
    /*  */
#define LAUNCH_COS(CAP, ASC) \
    fusion_cos_topk_warpsort_kernel<CAP, ASC, T, IdxT><<<grid, block, 0, stream>>>( \
        d_query_norm, d_data_norm, d_inner_product, d_index, \
        batch_size, len, k, output_vals, output_idx)

    if (select_min) {
        if      (capacity <= 64)  { LAUNCH_COS( 64, true); }
        else if (capacity <= 128) { LAUNCH_COS(128, true); }
        else if (capacity <= 256) { LAUNCH_COS(256, true); }
        else if (capacity <= 512)  { LAUNCH_COS(512, true); }
        else                       { LAUNCH_COS(1024, true); }
    } else {
        if      (capacity <= 64)   { LAUNCH_COS( 64, false); }
        else if (capacity <= 128) { LAUNCH_COS(128, false); }
        else if (capacity <= 256) { LAUNCH_COS(256, false); }
        else if (capacity <= 512)  { LAUNCH_COS(512, false); }
        else                       { LAUNCH_COS(1024, false); }
    }
#undef LAUNCH_COS
...
//    table_2D("h_topk_index", h_topk_index, n_query, k);
    ...
    int *d_index, *d_topk_index;
    {
        ...
        cudaMalloc(&d_topk_cos_dist, size_topk_dist);/*topk*/
    ...
    //
    {
        ...
        // cudaMemset((void*)d_topk_cos_dist, (int)0xEF, n_query * k * sizeof(float)) /*memset*/
...

    {
        ...
        INSITUANN::fusion_dist_topk_warpsort::fusion_cos_topk_warpsort(
            d_query_norm, d_data_norm, d_inner_product, d_index,
            n_query, n_batch, k,
